...
- ``/path/to/work/dir/enzymetrics_protein_catalog/final_reports/samples.csv`` for ingestion by the next module (ex. quality-checking)
.. ..
...
::
    └── workflow
        ├── Snakefile
        ├── enzymetrics_protein_catalog.py
        ├── utils.py
        └── __init__.py
- ``workflow/enzymetrics_protein_catalog.py``: Click-based CLI that wraps the ``snakemake`` and unit test generation commands for clean management of parameters, resources, and environment variables.
...
::
    python /path/to/camp_enzymetrics_protein_catalog/workflow/enzymetrics_protein_catalog.py \
        -w /path/to/camp_enzymetrics_protein_catalog/workflow/Snakefile \
        -d /path/to/work/dir \
...
::
    ...
    #!/bin/bash
    python /path/to/camp_enzymetrics_protein_catalog/workflow/enzymetrics_protein_catalog.py --slurm \
        -w /path/to/camp_enzymetrics_protein_catalog/workflow/Snakefile \
        -d /path/to/work/dir \
        -s /path/to/samples.csv
    EOF
