...
def run(workflow, work_dir, samples, unit_test, slurm):
    ...
    ryaml = join('/', *main_dir, 'configs', 'resources', 'bash.yaml')

    # Set location of compute cluster profile
    cluster_config = join('/', *main_dir, 'configs', 'resources', 'slurm.yaml') \
                     if slurm else None
